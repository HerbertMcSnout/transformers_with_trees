...
class Tree(Struct):
  ...
  def cast(self, new_class):
    "Maps in-place over this tree, setting the class of each node to new_class (which must subclass Tree)"
    assert issubclass(new_class, Tree), "new_class must be a subclass of Tree"
    self.__class__ = new_class
    if self.l: self.l.cast(new_class)
    if self.r: self.r.cast(new_class)
    return self
  ...
  def get_pos_embedding(self, embed_dim, params):
    ...
    def f_in(_, l, r):
      ...
      return (mu_l @ l) * (mu_r @ r) * step_scale
